...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-800 flex items-start justify-center py-6 px-3 md:py-10 md:px-8">
      {!user ? (
        <div className="max-w-md w-full bg-white p-6 md:p-8 rounded-2xl shadow-lg">
          <h2 className="text-2xl font-bold text-center mb-4">Welcome to ChatApp</h2>
          <p className="text-center text-gray-600 mb-6">Please log in or register to start chatting.</p>
          <div className="flex justify-center gap-3">
            <Link href="/login">
              <button className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg">Login</button>
            </Link>
            <Link href="/register">
        ...
        <div className="w-full max-w-7xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {/* Sidebar */}
          <aside className="col-span-1 bg-white border border-gray-200 p-4 rounded-2xl shadow-sm flex flex-col">
            ...
            <input
              ...
              className="w-full mb-3 px-3 py-2 rounded-lg border border-gray-300 focus:outline-none"
            />

            <div className="space-y-2 overflow-y-auto flex-1">
              {allUsers.length ? allUsers.map((u) => (
                <div key={u._id} className="flex justify-between p-2 rounded-lg hover:bg-gray-50">
                  <div>
                    <div className="font-medium">{u.name}</div>
                    <div className="text-xs text-gray-500">{u.email}</div>
                  </div>
                  <button onClick={() => openDmWith(u)} className="px-2 py-1 bg-blue-600 text-white rounded-md text-xs">Message</button>
                </div>
              )) : <div className="text-sm text-gray-500">No users found.</div>}
            </div>

            <h4 className="text-sm font-semibold mt-4 mb-2">Recent Chats</h4>
            <div className="space-y-2 overflow-y-auto max-h-40">
              {recentChats.length ? recentChats.map((r) => (
                <div key={r._id} className="flex justify-between p-2 rounded-lg hover:bg-gray-50">
                  <div>
                    <div className="font-medium">{r.name}</div>
                    <div className="text-xs text-gray-500">{r.email}</div>
                  </div>
                  <button onClick={() => openDmWith(r)} className="px-2 py-1 bg-blue-500 text-white rounded-md text-xs">Open</button>
                </div>
          ...
          {/* Rooms */}
          <section className="col-span-1 bg-white border border-gray-200 p-4 rounded-2xl shadow-sm flex flex-col">
            <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2 mb-3">
              ...
              />
              <button onClick={handleCreateRoom} className="px-3 py-2 bg-green-600 text-white rounded-lg text-sm">Create</button>
            </div>

            <div className="space-y-2 overflow-y-auto flex-1">
              {availableRooms.map((r) => (
                <div key={r.name} className="flex justify-between items-center p-2 border rounded-lg hover:bg-gray-50">
                  <div className="font-medium">{r.name}</div>
                  <button onClick={() => handleJoinRoom(r.name)} className="px-2 py-1 bg-blue-600 text-white rounded-md text-xs">Join</button>
                </div>
              ))}
            </div>
          </section>
          ...
          {/* Chat Area */}
          <main className="col-span-1 bg-white border border-gray-200 p-4 rounded-2xl shadow-sm flex flex-col">
            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
              ...
              <div className="flex gap-2">
                {chatType === "dm" && (
                  <button onClick={() => { setChatType("room"); setActiveDmUser(null); dmRoomRef.current = null; setDmMessages([]); }} className="px-3 py-1 bg-gray-300 rounded-md text-sm">Back</button>
                )}
                {joined && chatType === "room" && (
                  <button onClick={() => { setJoined(false); setRoom(""); setMessages([]); }} className="px-3 py-1 bg-gray-300 rounded-md text-sm">Leave</button>
                )}
              </div>
            </div>

            <div className="flex-1 overflow-y-auto p-3 mb-3 bg-gray-50 rounded-lg border border-gray-200">
